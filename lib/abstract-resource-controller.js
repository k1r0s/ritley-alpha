...
module.exports = AbstractResourceController = createClass({
  constructor: [inject.args(nodeProvider, configProvider), function(resourceIdentifier, node, config) {
    node.server.on("request", (req, res) =>
      this.requestAllowed(req, config.cfg.base, resourceIdentifier) &&
        this.processRequest(req, res));
    console.log(`listenning requests for ${config.cfg.base + resourceIdentifier}`);
  }],
