...
const NodeServer = createClass({
  ...
  constructor: [inject.args(configProvider), function(config) {
    ...
    staticDir && staticMiddleware = ecstatic({ root: `${staticDir}`, handleError: false });
    staticDir && console.log(`serving ${staticDir} as a static content`);

    basePath && this.server.on("request", (req, res) => !req.url.startsWith(basePath) && staticMiddleware(req, res));
  }]
});
